...
[[nodiscard]] csignal fourier_transform(csignal const &s, bool back = false) {
  ...
  auto to_ft = [=, &s](size_t j) {
    return std::accumulate(num_iterator{0}, num_iterator{std::size(s)}, cmplx{},
                           sum_up(j)) /
           div;
  };
...
template <typename F> static csignal signal_from_generator(size_t len, F gen) {
  auto r = csignal(len);
